...
filemanager::filemanager(int _in_count, int _out_count)
{
	...
	input.reserve(in_count);		/* Резервуємо місце у векторах */
...
{
	output[index].file_object.write((char*)&value, sizeof(int));		// Пишемо масив у файл
}
...
filemanager::~filemanager()
{
	for (int i = 0; i < in_count; i++)
	{
		input[i].file_object.close();				//Закриваємо файли
