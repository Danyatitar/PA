...
optional<size_t> bfs(const Game8Puzzle& game, Logger& logger) {
	logger.startTimer();
	std::size_t res = 0;
...
std::optional<std::size_t> AStar(const Game8Puzzle& game, Logger& logger) {
	...
	gamesSortedByF.insert({ 0, dist.insert({game, 0}).first });
	while (!gamesSortedByF.empty()) {
		...
		if (cur_game.moveRight()) {
			auto it = dist.find(cur_game);
			if ( it == dist.end()) {
				it = dist.insert({ cur_game, cur_dist + 1 }).first;
				gamesSortedByF.insert({ cur_game.CalcManhattanDist() + cur_dist + 1, it });
			}
			cur_game.moveLeft();
...
int main()
{
	...
	std::array<std::size_t, 9> matrix({ 2,1,EMPTY,
		                                6,7,4,
										5,8,3 });
	for (int i = 0; i < 1; i++) {
		...
		//cout << "Iteration count: " << logger_bfs.getIterationCount() << endl << endl << "Condition count: " << logger_bfs.getConditionsCount() << endl << endl << "Conditions in memory: "
			//<< logger_bfs.getConditionsInMemoryCount() << endl;

		cout << "Iteration count: " << logger_astar.getIterationCount() << endl << endl << "Condition count: " << logger_astar.getConditionsCount() << endl << endl << "Conditions in memory: "
			<< logger_astar.getConditionsInMemoryCount() << endl;
		...
		/*	
		 ...
		 out << i + 1 << " condition\t" << logger_bfs.getIterationCount() << '\t' << !sol1.has_value() << '\t'
			<< logger_bfs.getConditionsCount() << '\t' << logger_bfs.getConditionsInMemoryCount() << '\t';
		out<< "     " << '\t'
			<< logger_astar.getIterationCount() << '\t' << !sol2.has_value() << '\t'
			<< logger_astar.getConditionsCount() << '\t' << logger_astar.getConditionsInMemoryCount() << '\t';
